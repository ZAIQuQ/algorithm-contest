...
*	前缀异或和数组
*	将问题转为两两异或
*	对于任意单一位，n^2个两两异或中只有num(0) * num(1)个是1
*	故求和 sum{ bit[i][0] * bit[i][1] * 2^i }
...
*	树上启发式合并
	*	`根节点到任意节点路径上的轻边不超过O(log n)条`	
	*	当需要对每个子树搜索时，难点在于==每搜完一个子树后，计数器cnt似乎应该清零==
	*	而且无法保证每个结点都能维护一个计数器，故只能使用==一个动态计数器==

<img src="README.assets/image-20240405014848713.png" alt="image-20240405014848713" style="zoom:50%;" />


##### P9234
*	朴素搜索O(3^n)
*	折半搜索O(3^(n/2)) `meet-in-the-middle`
    * 就是将瓜的数量分成了N/2两份，然后折半搜索
    * 此外还需要一些剪枝
    * 可能需要手写Hash or 使用unordered_map